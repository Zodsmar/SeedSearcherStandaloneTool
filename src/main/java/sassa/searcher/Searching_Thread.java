                ...
                for (long upperBits = 0; upperBits < 1L << model.getBiomePrecision(); upperBits++) {
                    ...
                    for (HashMap.Entry<Feature, List<CPos>> feature : foundFeatures.entrySet()) {
                        ...
                        if ((structure = (RegionStructure) feature.getKey()) instanceof RegionStructure) {
                            for (CPos chunkPos : feature.getValue()) {
                                ...
                                //TODO clean this up... this is dumb I am also exhausted so whatever good enough for now
                                if (structure.getValidDimension() == Dimension.OVERWORLD) {
                                    if (!structure.canSpawn(chunkPos, sourceO)) continue;
                                } else if (structure.getValidDimension() == Dimension.NETHER) {
                                    if (!structure.canSpawn(chunkPos, sourceN)) continue;
                                } else if (structure.getValidDimension() == Dimension.END) {
                                    if (!structure.canSpawn(chunkPos, sourceE)) continue;
                                } else {
                                    //This theoretically should never be possible
                                    continue;
                                }
                    ...
                    for (int x = -model.getSearchRadius() + spawnPointX; x < model.getSearchRadius() + spawnPointX; x += model.getIncrementer()) {
                        for (int z = -model.getSearchRadius() + spawnPointZ; z < model.getSearchRadius() + spawnPointZ; z += model.getIncrementer()) {
                            ...
                            if (bModel.getExcludedBiomes().isEmpty() && validateSeed(worldSeed, bModel, bsModel, spawnedFeatures)) {
                                return;
                            }
                        }
                    }
                    ...
                    validateSeed(worldSeed, bModel, bsModel, spawnedFeatures);
                }
            }
        }
    }
...
//                RegionStructure.Data<?> upperBound = structure.at(model.getSearchRadius() >> 4 + spawnPointX >> 4, model.getSearchRadius() >> 4 + spawnPointZ >> 4);
    ...
    boolean validateSeed(long seed, BiomeList_Model bModel, BiomeSetList_Model bsModel, List<Feature> foundFeatures) {
        ...
        if (bModel.getIncludedBiomes().isEmpty() && bsModel.getIncludedBiomeSet().isEmpty() && foundFeatures.size() == model.getFeatureList().size()) {
